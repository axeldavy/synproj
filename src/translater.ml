...
let translate_exprs e ncall =
  ...
  (* Traduit une expression en Smt.Term, avec éventuellement des effets de bord *)
	...
	( match ident.kind with
	  ...
	  | _ -> let tlst = List.map translate_expr lst in
...
let translate_equs eqs ncall =
  List.iter (fun eq -> let texpr=translate_exprs eq.teq_expr ncall in
		       add_formula (fun n -> (Formula.make_lit Formula.Eq 
						[ Term.make_app (symbol_of (List.hd eq.teq_patt.tpatt_desc) ncall) [n];
						  texpr n ]))) eqs
